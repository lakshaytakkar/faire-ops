...
import { useJSBRetailers } from "@/lib/use-jsblueridge-data"
...
function deriveStatus(r: Row): RetailerStatus {
...
function AddRetailerDialog({ open, onClose, onSuccess }: { open: boolean; onClose: () => void; onSuccess: () => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null)
    try {
      const res = await fetch(`${API}?table=faire_retailers`, {
        method: "POST",
...
type EnrichRetailer = Row & { status: RetailerStatus }
...
function BulkEnrichModal({ open, onClose, retailers, onSaved }: {
  ...
  async function handleSaveAndNext() {
    ...
    setSaving(true)
    try {
      await fetch(`${API}?table=faire_retailers`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          action: "update",
              ...
              <div className="rounded-md border p-5 space-y-4">
                <div className="border-b pb-3">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    ...
                    {" \u00b7 "}
                    {formatCurrency(incomplete[currentIndex].total_spent_cents ?? 0)} spent
                  </p>
                </div>
...
export default function JSBlueridgeRetailersDirectoryPage() {
  const router = useRouter()
  const { retailers, totalCount, loading } = useJSBRetailers(5000)
  ...
  const retailersWithStatus = useMemo(
    () => retailers.map((r): Row & { status: RetailerStatus } => ({ ...r, status: deriveStatus(r) })),
    [retailers],
  )
  ...
  // Status counts derived from loaded data
  const statusCounts = useMemo(() => {
    const counts = { all: 0, active: 0, new: 0, vip: 0, inactive: 0 }
    retailersWithStatus.forEach((r) => {
      counts.all++
      counts[r.status]++
    })
    return counts
  ...
  const filtered = useMemo(() => {
    ...
    result = [...result].sort((a, b) => {
      switch (sortKey) {
        case "most_orders": return (b.total_orders ?? 0) - (a.total_orders ?? 0)
        case "most_revenue": return (b.total_spent_cents ?? 0) - (a.total_spent_cents ?? 0)
    ...
    return result
  }, [retailersWithStatus, statusFilter, countryFilter, searchQuery, sortKey])
  ...
  // Stats
  const stats = useMemo(() => {
    const totalRevenue = retailersWithStatus.reduce((s, r) => s + (r.total_spent_cents ?? 0), 0)
    return {
      total: totalCount || retailersWithStatus.length,
      active: statusCounts.active + statusCounts.vip,
      newCount: statusCounts.new,
      totalRevenue,
    }
  }, [retailersWithStatus, totalCount, statusCounts])

  // Has active filters?
  const hasActiveFilters = searchQuery || statusFilter !== "all" || countryFilter !== "all"
  ...
  const STAT_CARDS = [
    { label: "Total Retailers", value: stats.total.toLocaleString(), sub: "All wholesale buyers", icon: Users, iconBg: "bg-blue-50", iconColor: "text-blue-600" },
          ...
          <table className="w-full">
            ...
            <tbody>
              {paginatedRetailers.length === 0 ? (
                ...
                const revenue = (r.total_spent_cents ?? 0) / 100
                const avgOrder = (r.total_orders ?? 0) > 0 ? revenue / r.total_orders : 0
                ...
                return (
                  <tr
                    key={r.id}
                    onClick={() => router.push(`/jsblueridge/retailers/directory/${r.faire_retailer_id}`)}
                  ...
                  >
                    ...
                    {/* Orders */}
                    <td className="px-4 py-3.5 text-sm text-right tabular-nums">{r.total_orders ?? 0}</td>

                    {/* Revenue */}
                    <td className="px-4 py-3.5 text-sm text-right tabular-nums font-medium">{formatCurrency(r.total_spent_cents ?? 0)}</td>
