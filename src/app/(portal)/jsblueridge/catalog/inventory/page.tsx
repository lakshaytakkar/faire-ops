...
function getMinStock(product: Row): number {
...
export default function JSBlueridgeInventoryPage() {
  ...
  const totalUnits = faireProducts.reduce((sum: number, p: Row) => sum + (p.total_inventory ?? 0), 0)
  const lowStockItems = faireProducts.filter((p: Row) => (p.total_inventory ?? 0) > 0 && (p.total_inventory ?? 0) < getMinStock(p))
  const outOfStockItems = faireProducts.filter((p: Row) => (p.total_inventory ?? 0) === 0)
  const allLowStock = faireProducts.filter((p: Row) => (p.total_inventory ?? 0) < getMinStock(p))
  ...
  const toggleSelectAll = useCallback(() => {
    if (selectedIds.size === faireProducts.length) {
      ...
      setSelectedIds(new Set(faireProducts.map((p: Row) => p.id)))
    }
  ...
  const openBulkAdjust = useCallback(() => {
    const selected = faireProducts.filter((p: Row) => selectedIds.has(p.id))
  ...
  const statCards = [
    ...
    },
    {
      ...
      value: String(faireProducts.reduce((sum: number, p: Row) => sum + (p.variant_count ?? 0), 0)),
  ...
  return (
    <div className="space-y-6 max-w-[1440px] mx-auto w-full">
      ...
      {allLowStock.length > 0 && (
        <div className="rounded-lg border border-border/80 bg-card shadow-sm overflow-hidden">
          ...
          <div className="divide-y">
            {allLowStock.slice(0, 5).map((p: Row) => (
              <div key={p.id} className="flex items-center justify-between px-4 py-3 text-sm">
                <span className="font-medium">{p.name}</span>
                <div className="flex items-center gap-4">
                  <span className="text-muted-foreground">
                    Stock: <span className="font-bold text-amber-600">{p.total_inventory ?? 0}</span>
                    <span className="text-muted-foreground/60">/{getMinStock(p)}</span>
                  </span>
                  <Link
                    href="/jsblueridge/catalog/sourcing"
            ...
            <tbody>
              {faireProducts.length === 0 ? (
                ...
                sortedProducts.map((product: Row) => {
                  ...
                  const stock = product.total_inventory ?? 0
                          ...
                          <td colSpan={10} className="bg-muted/20 px-8 py-4">
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                              <div>
                                ...
                                <p className="font-medium">${((product.wholesale_price_cents ?? 0) / 100).toFixed(2)}</p>
                              </div>
                              <div>
                                <p className="text-xs text-muted-foreground">Retail Price</p>
                                <p className="font-medium">${((product.retail_price_cents ?? 0) / 100).toFixed(2)}</p>
                              </div>
                              <div>
      ...
      {adjustTarget && (
        <AdjustStockDialog
          productNames={adjustTarget.map((p: Row) => p.name)}
