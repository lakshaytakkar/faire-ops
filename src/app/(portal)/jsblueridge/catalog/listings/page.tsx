...
export default function JSBlueridgeListingsPage() {
  const router = useRouter()
  const { products, totalCount, loading } = useJSBProducts()
  ...
  // Accurate published count via API count query
  ...
  useEffect(() => {
    ...
    async function loadPublishedCount() {
      const sp = new URLSearchParams({
        ...
        count: "true",
        eq: JSON.stringify({ lifecycle_state: "PUBLISHED" }),
      })
      const res = await fetch(`/api/jsblueridge/data?${sp}`)
      const json = await res.json()
      if (!cancelled) setPublishedCount(json.count ?? 0)
    }
    loadPublishedCount()
  ...
  const filtered = useMemo(() => {
    let list = products
    if (statusFilter !== "all") {
      list = list.filter((p: any) => p.lifecycle_state === statusFilter)
    }
    if (searchQuery) {
    ...
    list.sort((a: any, b: any) => {
      ...
      switch (sortKey) {
        case "name":
          ...
          break
        case "price":
          cmp = (a.wholesale_price_cents ?? 0) - (b.wholesale_price_cents ?? 0)
          break
        case "stock":
          cmp = (a.total_inventory ?? 0) - (b.total_inventory ?? 0)
          break
        case "margin": {
          const mA = calcMargin((a.wholesale_price_cents ?? 0) / 100, (a.retail_price_cents ?? 0) / 100)
          const mB = calcMargin((b.wholesale_price_cents ?? 0) / 100, (b.retail_price_cents ?? 0) / 100)
          cmp = mA - mB
          break
        }
      }
  ...
  const avgWs = loadedCount > 0 ? products.reduce((s: number, p: any) => s + (p.wholesale_price_cents ?? 0), 0) / loadedCount / 100 : 0
  const totalInventory = products.reduce((s: number, p: any) => s + (p.total_inventory ?? 0), 0)

  if (loading) return <Skeleton />

  const statCards = [
    { label: "Total Products", value: String(totalProducts), trend: "JSBlueridge catalog", icon: <Package className="h-4 w-4" />, bg: "rgba(59,130,246,0.1)", color: "#3B82F6" },
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-0.5 text-sm text-muted-foreground">
            {totalProducts} product{totalProducts !== 1 ? "s" : ""} across the JSBlueridge catalog
          </p>
        </div>
        <PageResourcesButton pageRoute="/jsblueridge/catalog/listings" />
      </div>
            ...
            <tbody>
              {paginatedProducts.length === 0 ? (
                ...
                paginatedProducts.map((product: any) => {
                  ...
                  const retailPrice = (product.retail_price_cents ?? 0) / 100
                  ...
                  return (
                    <tr
                      key={product.id}
                      onClick={() => router.push("/jsblueridge/catalog/listings/" + product.faire_product_id)}
                    ...
                    >
                      ...
                      {/* Stock */}
                      <td className="px-4 py-3.5 text-sm text-center">
                        <span className={`font-medium ${(product.total_inventory ?? 0) === 0 ? "text-red-600" : (product.total_inventory ?? 0) < 10 ? "text-amber-600" : "text-foreground"}`}>
                          {(product.total_inventory ?? 0) === 0 ? "\u2014" : product.total_inventory}
                        </span>
                      </td>
