...
export default function JSBlueridgeOrdersPage() {
  const router = useRouter()
  const { orders, totalCount, loading: ordersLoading, refetch } = useJSBOrders()
  ...
  const [shipDialogOrder, setShipDialogOrder] = useState<JSBOrder | null>(null)
  ...
  // Fetch shipment tracking for in-transit orders via API proxy
  useEffect(() => {
    ...
    // Filter by in-transit order IDs via the "in" param
    sp.set("in", JSON.stringify({ order_id: inTransitIds }))

    fetch(`/api/jsblueridge/data?${sp}`)
      .then((res) => res.json())
      .then(({ data }) => {
  ...
  const ordersByState = useMemo(() => {
    const groups: Record<string, JSBOrder[]> = { NEW: [], PROCESSING: [], IN_TRANSIT: [], DELIVERED: [], CANCELED: [] }
  ...
  async function handleAccept(orderId: string, e: React.MouseEvent) {
    ...
    setActionError(null)
    try {
      const res = await fetch(`/api/jsblueridge/orders/${orderId}/accept`, { method: "POST" })
  ...
  async function handleShip() {
    ...
    setShipping(true)
    try {
      const res = await fetch(`/api/jsblueridge/orders/${shipDialogOrder.faire_order_id}/ship`, {
        method: "POST",
    ...
    <div className="space-y-5 max-w-[1440px] mx-auto w-full">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm text-muted-foreground">{stats.total} orders for JSBlueRidge Toys</p>
        </div>
        <PageResourcesButton pageRoute="/jsblueridge/orders" />
      </div>
                  ...
                  <tbody>
                    {paginatedOrders.length === 0 ? (
                      ...
                      return (
                        <tr
                          key={order.id}
                          onClick={() => router.push("/jsblueridge/orders/" + order.faire_order_id)}
                        ...
                        >
                          ...
                          <td className="px-4 py-3.5 text-sm text-right">
                            <Link href={"/jsblueridge/orders/" + order.faire_order_id} onClick={(e) => e.stopPropagation()} className="text-xs text-primary font-medium hover:underline">
                              View
                            </Link>
                          </td>
                        </tr>
                          ...
                          {visibleOrders.map((order) => (
                            ...
                            >
                              ...
                              <div className="flex items-center justify-between mt-1">
                                <span className="text-xs text-muted-foreground">JSBlueRidge</span>
                                {order.source && (
