...
import { useJSBOrders } from "@/lib/use-jsblueridge-data"
...
export default function JSBFulfillmentPage() {
  const { orders, loading } = useJSBOrders("PROCESSING")
  ...
  async function markShipped(orderId: string, faireOrderId: string) {
    setShippingId(orderId)
    try {
      await fetch(`/api/jsblueridge/orders/${faireOrderId}/ship`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ tracking_code: "", carrier: "UPS" }),
      })
                    ...
                    <tr key={order.id} className="border-b last:border-b-0 hover:bg-muted/20 transition-colors">
                      ...
                      <td className="px-4 py-3.5 text-sm text-right">
                        {isMarkedShipped ? (
                          ...
                          <button
                            onClick={() => markShipped(order.id, order.faire_order_id)}
                            disabled={shippingId === order.id}
                            className="h-8 gap-1 text-xs inline-flex items-center justify-center rounded-md bg-primary text-primary-foreground px-3 font-medium hover:bg-primary/90 transition-colors disabled:opacity-50"
                          >
