...
export default function JSBShipmentsPage() {
  ...
  // Fetch all shipped/in-transit orders + tracking data via API
  async function fetchShipments() {
    setLoading(true)

    // 1. Get shipped orders via API
    const { data: shippedOrders } = await jsbFetch({
      table: "faire_orders",
      select: "faire_order_id, display_id, store_id, state, total_cents, item_count, shipping_address, faire_created_at, raw_data",
      eq: JSON.stringify({ state: "IN_TRANSIT" }),
      limit: "500",
    })

    // Also fetch delivered and pre-transit
    const { data: deliveredOrders } = await jsbFetch({
      table: "faire_orders",
      select: "faire_order_id, display_id, store_id, state, total_cents, item_count, shipping_address, faire_created_at, raw_data",
      eq: JSON.stringify({ state: "DELIVERED" }),
      limit: "500",
    })

    const { data: preTransitOrders } = await jsbFetch({
      table: "faire_orders",
      select: "faire_order_id, display_id, store_id, state, total_cents, item_count, shipping_address, faire_created_at, raw_data",
      eq: JSON.stringify({ state: "PRE_TRANSIT" }),
      limit: "500",
    })

    const allOrders = [...(shippedOrders ?? []), ...(deliveredOrders ?? []), ...(preTransitOrders ?? [])]
    const oMap: Record<string, OrderInfo> = {}
    for (const o of allOrders) oMap[o.faire_order_id] = o
    setOrderMap(oMap)

    // 2. Get tracking entries via API
    const { data: trackingData } = await jsbFetch({
    ...
    const trackingEntries = (trackingData ?? []) as ShipmentTracking[]
    ...
    // Then add orders without tracking entries
    for (const o of allOrders) {
      if (seenOrderIds.has(o.faire_order_id)) continue
      const raw = o.raw_data as Record<string, unknown> | undefined
      const rawShipments = (raw?.shipments ?? []) as Array<Record<string, unknown>>
      const firstShipment = rawShipments[0]
  ...
  const tabFiltered = useMemo(() => {
    ...
    if (activeTab === "delayed") return shipments.filter((s) => s.is_delayed)
  ...
  const avgTransitDays = useMemo(() => {
    const delivered = shipments.filter((s) => s.status === "delivered" && s.transit_days > 0)
  ...
  const tabs: { key: TabKey; label: string; count: number }[] = [
    ...
    { key: "exception", label: "Exceptions", count: shipments.filter((s) => s.status === "exception").length },
  ]
                  ...
                  return (
                    <tr key={shipment.id} className="border-b last:border-b-0 hover:bg-muted/20 transition-colors">
                      ...
                      <td className="px-4 py-3.5 text-sm">
                        <Link
                          href={`/jsblueridge/orders/${shipment.order_id}`}
                      ...
                      <td className="px-4 py-3.5 text-sm">
                        <div>
                          ...
                          <div className="flex items-center gap-1 mt-0.5">
                            ...
                            {shipment.tracking_code && <CopyButton text={shipment.tracking_code} />}
                          </div>
                        </div>
                      </td>
