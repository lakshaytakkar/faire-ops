...
import { useJSBOrderStats } from "@/lib/use-jsblueridge-data"
...
const JSB_COLOR = "#3b82f6"
const JSB_STORE_NAME = "JSBlueRidge Toys"
...
export default function JSBlueridgeAnalyticsPage() {
  const [period, setPeriod] = useState<PeriodKey>("30d")

  const { stats: allTimeStats, loading: statsLoading } = useJSBOrderStats()

  /* -------------- Period-scoped revenue + order count -------------- */
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      // Fetch orders, optionally filtered by period
      const params: Record<string, string> = {
        table: "faire_orders",
        select: "total_cents",
        limit: "10000",
      }
      // For period filtering, we fetch all and filter client-side
      // since the API proxy may not support gte directly
      const { data } = await jsbFetch(params)
      if (cancelled) return

      const rows = (data ?? []).filter((r: Row) => {
        if (!startIso) return true
        return r.faire_created_at && r.faire_created_at >= startIso
      })

      const totalOrders = rows.length
      const totalRevenue = rows.reduce(
        (s: number, r: Row) => s + (typeof r.total_cents === "number" ? r.total_cents : 0),
        0,
      )
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      const filtered = ((data ?? []) as OrderRowLite[]).filter((r) => {
        if (!startIso) return true
        return r.faire_created_at && r.faire_created_at >= startIso
      })
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      const filtered = ((data ?? []) as OrderRowLite[]).filter(
        (r) => r.faire_created_at && r.faire_created_at >= sixMonthsAgo.toISOString(),
      )
      setChartOrders(filtered)
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      const filtered = ((data ?? []) as ViewRow[]).filter(
        (v) => v.view_date >= since.toISOString().slice(0, 10),
      )
      setViews(filtered)
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      // Aggregate bestsellers client-side from orders + products
      const { data: ordersData } = await jsbFetch({
        table: "faire_orders",
        select: "raw_data,faire_created_at",
        limit: "10000",
      })

      const { data: productsData } = await jsbFetch({
        table: "faire_products",
        select: "faire_product_id,name,primary_image_url",
        limit: "5000",
      })
      if (cancelled) return

      const prodMap = new Map<string, { name: string; imageUrl: string | null }>()
      for (const p of (productsData ?? []) as Row[]) {
        prodMap.set(p.faire_product_id, {
          name: p.name ?? "Unknown",
          imageUrl: p.primary_image_url ?? null,
        })
      }

      const agg = new Map<string, { orders: number; revenueCents: number }>()
      for (const order of (ordersData ?? []) as Row[]) {
        if (startIso && order.faire_created_at && order.faire_created_at < startIso) continue
        const raw = order.raw_data as Record<string, unknown> | null
        const items = (raw?.items as OrderItem[] | undefined) ?? []
        for (const item of items) {
          if (!item.product_id) continue
          const existing = agg.get(item.product_id) ?? { orders: 0, revenueCents: 0 }
          existing.orders += 1
      ...
      const rows: BestsellerRow[] = Array.from(agg.entries())
        .map(([pid, d]) => ({
          productId: pid,
          name: prodMap.get(pid)?.name ?? "Unknown product",
          orders: d.orders,
          revenueCents: d.revenueCents,
          imageUrl: prodMap.get(pid)?.imageUrl ?? null,
        }))
  ...
  useEffect(() => {
    ...
    run()
    return () => { cancelled = true }
  }, [bestsellers.length, bestsellersLoading])
  ...
  const dailySalesByStore: DailyStorePoint[] = useMemo(() => {
    ...
    for (const order of chartOrders) {
      ...
      if (key in dayMap) {
        dayMap[key]["jsb"] =
          (dayMap[key]["jsb"] ?? 0) + (order.total_cents ?? 0)
      }
    }
  ...
  return (
    <div className="max-w-[1440px] mx-auto w-full space-y-5">
      ...
      <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-sm text-muted-foreground">
            {JSB_STORE_NAME} performance overview
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="rounded-lg border border-border/80 bg-card shadow-sm p-5 flex items-start justify-between">
          <div>
            ...
            <p className="text-2xl font-bold font-heading mt-2">1</p>
            <p className="text-xs text-muted-foreground mt-1">{JSB_STORE_NAME}</p>
          </div>
          <div
            className="h-9 w-9 rounded-lg flex items-center justify-center"
      ...
      {/* ---- Bestsellers (single-brand, no top stores card) ---- */}
      ...
      <div className="rounded-lg border border-border/80 bg-card shadow-sm overflow-hidden">
        <div className="flex items-center justify-between border-b px-5 py-3.5">
          ...
          <span className="flex items-center gap-1 text-[10px] text-muted-foreground">
            <span className="w-2.5 h-2.5 rounded-sm" style={{ backgroundColor: JSB_COLOR }} />
            {JSB_STORE_NAME}
          </span>
        </div>
        <div className="p-5">
      ...
      <div className="rounded-lg border border-border/80 bg-card shadow-sm overflow-hidden">
        <div className="flex items-center justify-between border-b px-5 py-3.5">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              {JSB_STORE_NAME} &middot; {viewLines.length} days tracked
            </p>
          </div>
          <span className="flex items-center gap-1 text-[10px] text-muted-foreground">
            <span className="w-2.5 h-[3px] rounded-full" style={{ backgroundColor: JSB_COLOR }} />
            {JSB_STORE_NAME}
          </span>
        </div>
        <div className="p-5">
                ...
                <div className="flex-1 relative h-64">
                  ...
                  >
                    {(() => {
                      ...
                      for (const row of viewLines) {
                        for (const sid of Object.keys(row.stores)) storeIds.add(sid)
                      }
                      return Array.from(storeIds).map((storeId) => {
                        const pointsArray: [number, number][] = viewLines.map(
                          (row, i) => {
                            const val = row.stores[storeId] ?? 0
                        ...
                        <div key={row.date} className="flex-1 group relative">
                          <div className="absolute top-0 left-1/2 -translate-x-1/2 hidden group-hover:block z-20">
                            <div className="bg-foreground text-background text-[10px] rounded px-2 py-1.5 shadow-lg whitespace-nowrap">
                              ...
                              {Object.entries(row.stores).map(([sid, count]) => (
                                <p key={sid} className="flex items-center gap-1">
                                  <span
                                    className="w-2 h-[2px] rounded"
                                    style={{ backgroundColor: JSB_COLOR }}
                                  />
                                  Views: {count}
                                </p>
                              ))}
      ...
      {/* ========================================================== */}
      {/*  US MAP (GEOGRAPHY) + state-by-revenue table                */}
      {/* ========================================================== */}
      <div className="rounded-lg border border-border/80 bg-card shadow-sm overflow-hidden">
        <div className="px-5 py-3.5 border-b flex items-center justify-between">
          <div>
      ...
      <div className="rounded-lg border border-border/80 bg-card shadow-sm px-5 py-4">
        ...
        <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm">
          <Link
            href="/jsblueridge/overview"
            className="inline-flex items-center gap-1 text-primary font-medium hover:underline"
          >
            Overview <ArrowRight className="h-3 w-3" />
          </Link>
          <Link
            href="/jsblueridge/overview/reports"
            className="inline-flex items-center gap-1 text-primary font-medium hover:underline"
          >
