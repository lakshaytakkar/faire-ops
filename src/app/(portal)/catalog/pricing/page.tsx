...
export default function PricingPage() {
  ...
  const stats = useMemo(() => {
    ...
    const margins = faireProducts.map((p) =>
      calcMargin(p.retail_price_cents / 100, p.wholesale_price_cents / 100),
    )
    const avgMargin = margins.reduce((s, m) => s + m, 0) / count
    const avgWholesale =
      faireProducts.reduce((s, p) => s + p.wholesale_price_cents, 0) / count / 100
    const avgRetail =
      faireProducts.reduce((s, p) => s + p.retail_price_cents, 0) / count / 100
    const totalRetail =
      faireProducts.reduce((s, p) => s + p.retail_price_cents, 0) / 100
  ...
  const sortedProducts = useMemo(() => {
    return [...faireProducts].sort((a, b) => {
      ...
      switch (sortKey) {
        case "margin":
          return dir * (calcMargin(a.retail_price_cents / 100, a.wholesale_price_cents / 100) - calcMargin(b.retail_price_cents / 100, b.wholesale_price_cents / 100))
        case "price":
          return dir * (a.wholesale_price_cents - b.wholesale_price_cents)
        default:
          return 0
      }
    })
  ...
  const toggleAll = () => {
    ...
    else setSelected(new Set(faireProducts.map((p) => p.id)))
  }
  ...
  const runAiOptimizer = useCallback(async () => {
    ...
    const lowMargin = faireProducts
      .filter((p) => calcMargin(p.retail_price_cents / 100, p.wholesale_price_cents / 100) < 50)
      .map((p) => {
        const ws = p.wholesale_price_cents / 100
        const rt = p.retail_price_cents / 100
  ...
  const distribution = useMemo(() => {
    const high = faireProducts.filter(
      (p) => calcMargin(p.retail_price_cents / 100, p.wholesale_price_cents / 100) > 60,
    ).length
    const med = faireProducts.filter((p) => {
      const m = calcMargin(p.retail_price_cents / 100, p.wholesale_price_cents / 100)
      return m >= 40 && m <= 60
    }).length
    const low = faireProducts.filter(
      (p) => calcMargin(p.retail_price_cents / 100, p.wholesale_price_cents / 100) < 40,
    ).length
    return { high, med, low }
  }, [faireProducts])
  ...
  const statCards = [
    {
      ...
      trend: `Across ${count} products`,
      icon: <TrendingUp className="h-4 w-4" />,
      bg: marginSc.bg,
      color: marginSc.color,
    },
    {
      label: "Avg Wholesale",
      value: fmt(stats.avgWholesale),
      trend: "Wholesale average",
    ...
    {
      label: "Avg Retail",
      value: fmt(stats.avgRetail),
      trend: "Retail average",
      icon: <BarChart3 className="h-4 w-4" />,
      bg: "rgba(139,92,246,0.1)",
      color: "#8B5CF6",
    },
    {
      label: "Total Portfolio Retail Value",
      value: fmt(stats.totalRetail),
  ...
  return (
    <div className="space-y-6 max-w-[1440px] mx-auto w-full">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="mt-0.5 text-sm text-muted-foreground">
            Wholesale pricing analysis and optimization
          </p>
        </div>
                    ...
                    <Fragment key={p.id}>
                      ...
                      >
                        ...
                        <td className="px-4 py-3.5 text-sm text-center">
                          <span className={`border-0 text-xs font-medium px-2 py-0.5 rounded-full ${marginBadgeCls(margin)}`}>
                            {margin.toFixed(1)}%
                          </span>
                        </td>
                        <td className="px-4 py-3.5 text-sm text-center text-muted-foreground font-medium">{markup}x</td>
                        <td className="px-4 py-3.5 text-sm text-right text-muted-foreground">{p.total_inventory}</td>
                      </tr>
                          ...
                          <td colSpan={10} className="px-8 py-4">
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                              <div>
                                ...
                                <p className="font-medium">{p.variant_count}</p>
                              </div>
                              <div>
                                <p className="text-xs text-muted-foreground">Min Order Qty</p>
                                <p className="font-medium">{p.minimum_order_quantity}</p>
                              </div>
                              <div>
                                <p className="text-xs text-muted-foreground">Made In</p>
                                <p className="font-medium">{p.made_in_country ?? "--"}</p>
                              </div>
                              <div>
                                <p className="text-xs text-muted-foreground">Sale State</p>
                                <p className="font-medium">{p.sale_state}</p>
                              </div>
                            </div>
                          </td>
                        </tr>
      ...
      {aiVisible && (
        <div className="rounded-md border bg-card p-5">
          ...
          {aiLoading ? (
            ...
            <div className="rounded-md border bg-muted/20 p-4 text-sm whitespace-pre-wrap leading-relaxed text-foreground">
              {aiResult}
            </div>
          ) : null}
        </div>
      ...
      <p className="text-xs text-muted-foreground text-center">
        Showing {faireProducts.length} products
      </p>
    </div>
  )
}
