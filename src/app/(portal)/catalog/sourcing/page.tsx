...
/* ------------------------------------------------------------------ */
/*  Scraper categories & sources (inline, no pipeline-data import)     */
/* ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------ */
/*  Mock scraper products (for scraper simulation UI)                  */
/* ------------------------------------------------------------------ */
...
const SCRAPE_HISTORY = [
  ...
  { date: "Mar 31, 2026", source: "WonATrading", productsFound: 22, highSignal: 7, status: "completed" as const },
  { date: "Mar 30, 2026", source: "Alibaba", productsFound: 27, highSignal: 9, status: "completed" as const },
]

/* ------------------------------------------------------------------ */
/*  Today's source rotation                                            */
/* ------------------------------------------------------------------ */

const SOURCE_ROTATION = ["Minea", "AliExpress", "ToyNetwork", "WonATrading", "Alibaba", "Faire New Arrivals"]

function getTodaysSource(): string {
  const day = new Date().getDay()
  return SOURCE_ROTATION[day % SOURCE_ROTATION.length]
}
...
export default function SourcingPage() {
  ...
  const filtered = useMemo(() => {
    return scraperProducts.filter((p) => {
  ...
  const avgMargin =
    totalFound > 0
      ? (scraperProducts.reduce((sum, p) => sum + p.margin, 0) / totalFound).toFixed(1)
      : "0.0"
  ...
  function queueProduct(id: string) {
    ...
    <div className="space-y-6 max-w-[1440px] mx-auto w-full pb-20">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm text-muted-foreground">Discover trending products</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          >
            ...
            {isRunning ? "Scanning..." : "Run Scrape"}
          </Button>
        </div>
      </div>

      {/* -- Manual Import Form (collapsible) -- */}
      {showManualForm && (
        <div className="rounded-md border bg-card p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
            <div className="space-y-1.5">
              ...
              <Input
                placeholder="e.g. Boho Rattan Lamp"
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-muted-foreground">COGS Estimate ($)</label>
              <Input
                type="number"
                placeholder="0.00"
                className="h-8 text-sm"
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-muted-foreground">Target Brand</label>
              <Select value={manualBrand} onValueChange={(v) => setManualBrand(v ?? "")}>
          ...
          <div className="flex items-center gap-2 pt-1">
            <Button size="sm" className="h-8 text-sm gap-1.5" onClick={handleManualAdd} disabled={!manualName.trim()}>
              <ListPlus className="h-3.5 w-3.5" />
              Add to Queue
            </Button>
            <Button size="sm" variant="ghost" className="h-8 text-sm" onClick={() => setShowManualForm(false)}>
              Cancel
            </Button>
          </div>
        </div>
      ...
      <div className="rounded-md border bg-card overflow-hidden">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-b px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <Select value={source} onValueChange={(v) => setSource(v ?? "All Sources")}>
          ...
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
            ...
            <span className="font-medium text-foreground">{todaysSource}</span>
          </div>
        </div>
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                [...filtered].sort((a, b) => {
                  ...
                  if (sortKey === "price") return (a.wsPrice - b.wsPrice) * dir
                  return 0
                }).map((product) => {
                    ...
                    >
                      <td className="px-4 py-3.5 text-sm">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-md bg-gradient-to-br from-amber-50 to-amber-100 flex items-center justify-center shrink-0">
                      ...
                      <td className="px-4 py-3.5 text-sm text-muted-foreground max-w-[200px] truncate">
                        {product.trendSignal}
                      </td>
                      ...
                      <td className="px-4 py-3.5 text-sm text-center">
                        <span className={`border-0 text-xs font-medium px-2 py-0.5 rounded-full ${SIGNAL_STYLES[product.signal]}`}>
                          {product.signal.charAt(0).toUpperCase() + product.signal.slice(1)}
                        </span>
                      </td>
                      <td className="px-4 py-3.5 text-sm text-right">
                        {isQueued ? (
                          ...
                          <Button
                            ...
                            onClick={() => queueProduct(product.id)}
                          >
                            Queue
                          </Button>
                        )}
      ...
      <div className="rounded-md border bg-card overflow-hidden">
        ...
        <div className="overflow-auto">
          <table className="w-full">
            <thead>
              <tr className="border-b bg-muted/40">
                <th className="px-4 py-3 text-xs font-medium text-muted-foreground tracking-wide uppercase text-left">Date</th>
                <th className="px-4 py-3 text-xs font-medium text-muted-foreground tracking-wide uppercase text-left">Source</th>
                <th className="px-4 py-3 text-xs font-medium text-muted-foreground tracking-wide uppercase text-right">Products Found</th>
                <th className="px-4 py-3 text-xs font-medium text-muted-foreground tracking-wide uppercase text-right">High Signal</th>
                <th className="px-4 py-3 text-xs font-medium text-muted-foreground tracking-wide uppercase text-center">Status</th>
              </tr>
            </thead>
            <tbody>
              {SCRAPE_HISTORY.map((entry, i) => (
                <tr key={i} className="border-b last:border-b-0 hover:bg-muted/20 transition-colors">
                  <td className="px-4 py-3.5 text-sm font-medium">{entry.date}</td>
                  <td className="px-4 py-3.5 text-sm text-muted-foreground">{entry.source}</td>
                  <td className="px-4 py-3.5 text-sm text-right">{entry.productsFound}</td>
                  <td className="px-4 py-3.5 text-sm text-right">{entry.highSignal}</td>
                  <td className="px-4 py-3.5 text-sm text-center">
      ...
      {/* -- Queue Summary Bar (fixed bottom) -- */}
      {queuedIds.size > 0 && (
        <div className="fixed bottom-0 left-0 right-0 z-40 border-t bg-card/95 backdrop-blur supports-[backdrop-filter]:bg-card/80">
          <div className="max-w-[1440px] mx-auto flex items-center justify-between px-6 py-3">
            <p className="text-sm font-medium">
              {queuedIds.size} product{queuedIds.size !== 1 ? "s" : ""} queued
            </p>
            <div className="flex items-center gap-3">
              <Button variant="ghost" size="sm" className="h-8 text-xs" onClick={handleClearQueue}>
                Clear Queue
              </Button>
              <Link
                href="/catalog/publishing-queue"
