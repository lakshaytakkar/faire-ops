...
export interface GoyoDrawerFormProps {
  open: boolean
  onClose: () => void
  title: string
  subtitle?: string
...
export function GoyoDrawerForm({
  open,
  onClose,
  title,
  subtitle,
    ...
    <EditDrawer
      ...
      footer={
        <>
          ...
          <Button type="submit" form="goyo-drawer-form" size="sm" disabled={pending}>
            {pending ? "Saving…" : id ? "Save" : "Create"}
          </Button>
        </>
      }
    >
      <form id="goyo-drawer-form" onSubmit={handleSubmit} className="space-y-4">
        {fields.map((f) => (
