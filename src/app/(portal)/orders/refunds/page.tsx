...
function getIssueType(order: FaireOrder): IssueType {
...
function getAffectedItems(order: FaireOrder): AffectedItem[] {
...
export default function RefundsPage() {
  ...
  const [allOrders, setAllOrders] = useState<FaireOrder[]>([])
    ...
    supabaseB2B
      ...
      .then(({ data, error }) => {
        if (error) {
          ...
          const issueOrders = (data ?? []).filter((o) => {
  ...
  const orders = useMemo(() => {
    if (activeBrand === "all") return allOrders
    return allOrders.filter((o) => o.store_id === activeBrand)
  }, [allOrders, activeBrand])
  ...
  function getStoreColor(storeId: string): string {
    return stores.find((s) => s.id === storeId)?.color ?? "#94a3b8"
  }
                  ...
                  filteredOrders.map((order) => {
                    ...
                    return (
                      <IssueRow
                        ...
                        storeName={getStoreName(order.store_id)}
                        storeColor={getStoreColor(order.store_id)}
