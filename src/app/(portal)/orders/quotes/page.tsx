...
export default function QuotesPage() {
  ...
  const groupedByOrder = useMemo(() => {
    ...
    // Sort order keys by first quote created_at desc
  ...
  async function handleBulkRequestQuotes() {
    ...
    try {
      ...
      await Promise.all(
        noneOrders.map((o) =>
          fetch("/api/faire/orders/request-quotes", {
            method: "POST",
  ...
  async function handleApproveQuote(quoteId: string) {
    ...
    try {
      await fetch("/api/faire/orders/approve-quote", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
              ...
              <div key={orderId} className="rounded-lg border border-border/80 bg-card shadow-sm overflow-hidden">
                ...
                <div className="px-4 py-3 border-b bg-muted/30 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    {order?.store_id && <span className="text-xs text-muted-foreground">Store: {order.store_id.slice(0, 8)}</span>}
