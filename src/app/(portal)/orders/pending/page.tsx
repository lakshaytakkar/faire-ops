...
export default function PendingOrdersPage() {
  ...
  const { orders, loading: ordersLoading } = useOrders(storeId, "NEW")
  ...
  const totalValueCents = orders.reduce((sum, o) => sum + o.total_cents, 0)
  ...
  const avgItems = useMemo(() => {
    if (pendingCount === 0) return "0"
    return (orders.reduce((sum, o) => sum + o.item_count, 0) / pendingCount).toFixed(1)
  }, [orders, pendingCount])
                  ...
                  >
                    ...
                    <td className="px-4 py-3.5 text-sm">
                      <span className="flex items-center gap-1.5">
                        <span
                          ...
                          style={{ backgroundColor: getStoreColor(order.store_id) }}
                        />
                    ...
                    <td className="px-4 py-3.5 text-sm text-right">
                      <span className="flex items-center justify-end gap-2">
                        ...
                        <button
                          className="h-9 gap-1.5 text-sm inline-flex items-center justify-center rounded-md bg-primary text-primary-foreground px-3 font-medium hover:bg-primary/90 transition-colors"
                        >
                          Accept
                        </button>
                      </span>
