...
export function SmsTabs({
  ...
  return (
    <>
      ...
      {tab === "templates" && (
        <DetailCard title="SMS templates">
          {templates.length === 0 ? (
            <EmptyState
              icon={FileText}
              title="No templates yet"
              description="Short-form SMS templates used by automations live here."
            />
          ) : (
                ...
                <tbody>
                  {templates.map((t) => (
                    <tr key={t.id} className="border-b border-border/60 last:border-0">
                      ...
                      <td className="px-4 py-2.5 text-muted-foreground line-clamp-1 max-w-md">
                        {t.body ?? "—"}
                      </td>
        ...
        <DetailCard title="Automations">
          {automations.length === 0 ? (
            <EmptyState
              ...
              description="Triggered SMS flows tied to user events appear here once configured."
            />
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
        ...
        <DetailCard title="Delivery logs">
          {logs.length === 0 ? (
            <EmptyState
              icon={MessageSquare}
              title="No SMS logs"
              description="Delivery attempts and their status appear here once SMS sends start flowing."
            />
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {logs.map((l) => (
                    <tr key={l.id} className="border-b border-border/60 last:border-0">
                      <td className="px-4 py-2.5 font-medium">{l.recipient_phone ?? "—"}</td>
                      <td className="px-4 py-2.5 text-muted-foreground line-clamp-1 max-w-md">
                        {l.message ?? "—"}
                      </td>
                      <td className="px-4 py-2.5">
