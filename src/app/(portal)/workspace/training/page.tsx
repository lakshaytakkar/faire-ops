...
export default function TrainingPage() {
  ...
  useEffect(() => {
    ...
    seeded.current = true
    ;(async () => {
      const [videoData] = await Promise.all([fetchVideos(), fetchSops()])
      if (videoData.length === 0) {
        await supabase.from("training_videos").insert(SEED_VIDEOS)
        fetchVideos()
      }
    })()
  }, [fetchVideos, fetchSops])
  ...
  return (
    <div className="space-y-6 max-w-[1440px] mx-auto w-full">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm text-muted-foreground">
            Team training videos and standard operating procedures
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="rounded-lg border border-border/80 bg-card shadow-sm p-5 flex items-start justify-between">
          <div>
            <p className="text-xs font-medium text-muted-foreground">SOPs</p>
      ...
      <div>
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-lg font-semibold font-heading">Training Videos</h2>
        </div>
        ...
        {loadingVideos ? (
          ...
          <div className="rounded-lg border border-border/80 bg-card shadow-sm p-8 text-center">
            ...
            <p className="text-sm font-medium text-muted-foreground">No training videos found</p>
            <p className="text-xs text-muted-foreground mt-1">
              {activeCategory === "All"
                ? "Add your first training video to get started."
                : `No videos in the "${activeCategory}" category.`}
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {filteredVideos.map((video, index) => (
              <div
                key={video.id}
                className="rounded-lg border border-border/80 bg-card shadow-sm overflow-hidden hover:shadow-md transition-shadow group relative cursor-pointer"
              ...
              >
                ...
                >
                  ...
                  {/* Duration Badge */}
                  <span className="absolute bottom-2 right-2 bg-black/60 text-white text-xs font-medium px-2 py-0.5 rounded">
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center gap-2 mt-3 flex-wrap">
                    ...
                    {video.is_required ? (
                      ...
                      <span className="border-0 text-xs font-medium px-2 py-0.5 rounded-full bg-slate-50 text-slate-600">
                        Optional
                      </span>
                    )}
                  </div>
      ...
      <div>
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-lg font-semibold font-heading">Standard Operating Procedures</h2>
        </div>
        ...
        {loadingSops ? (
          ...
          <div className="rounded-lg border border-border/80 bg-card shadow-sm p-8 text-center">
            ...
            <p className="text-sm font-medium text-muted-foreground">No SOPs available yet</p>
            <p className="text-xs text-muted-foreground mt-1">
