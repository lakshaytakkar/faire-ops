 ...
 *
 * Each cell is `w-12 h-12` with `border-b border-white/15` so the cells
...
export function BrandDock() {
  ...
  if (storesLoading) {
    return (
      <aside className="shrink-0 w-12 bg-black flex flex-col border-l border-r border-white/15">
        {[1, 2, 3, 4, 5, 6].map((i) => (
          <div key={i} className="w-12 h-12 border-b border-white/15 bg-white/[0.04] animate-pulse" />
        ))}
      </aside>
    )
  }
  ...
  return (
    <aside className="shrink-0 w-12 bg-black flex flex-col border-l border-r border-white/15">
      ...
      <button
        ...
        className={cn(
          "relative flex items-center justify-center w-12 h-12 border-b border-white/15 group transition-all bg-zinc-800",
          activeBrand === "all" && "ring-1 ring-inset ring-white/50"
        )}
        title="All Stores"
      >
